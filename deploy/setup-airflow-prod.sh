...
# $SRC/logs — симлинк на боевой каталог логов (создаётся ниже). Обе команды
# выше в него НЕ заходят: chgrp/chmod -R по умолчанию симлинки не разыменовывают
# (-P). Если когда-нибудь захочется добавить сюда -L или chown -R по логам —
# не надо: на тесте рекурсия по логам вешала запуск на минуты.
...
while read oldrev newrev ref; do
    ...
    if PARSE_LOG=\$(REQUIRE_AIRFLOW=1 bash "$SRC/deploy/check-dags.sh" "$SRC" 2>&1); then
        ...
        echo "\$PARSE_LOG"
        echo "!! Код УЖЕ в $SRC/dags. ОТКАТИТЬ:                     !!"
        echo "!!   FROM=<прошлый тег prod-*> bash deploy/deploy-prod.sh !!"
        echo "!! Сюда доходят только те, кто обошёл pre-receive.       !!"
        echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    fi

    # Рестарт от проверки НЕ зависит: раньше он висел в ветке «проверка прошла»,
    # и провал оставлял систему расщеплённой — на диске новый код, в памяти
    # старый. Незаметно это потому, что LocalExecutor форкает задачи из
    # долгоживущего воркера: первый выполненный таск импортирует Functions.*,
    # все следующие форки наследуют ЭТОТ импорт, и правка на диске не работает
    # до рестарта планировщика. Не рестартовать смысла нет: код всё равно лежит
    # в DAGS_FOLDER и будет перечитан.
    # А вот от переключения зависит: до cutover (нет drop-in) код просто
    # раскладывается, работающий на старой папке прод не трогаем.
