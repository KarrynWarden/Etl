...
### Выкладка из конструктора (кнопка «Выложить на прод»)

По умолчанию её нет: сервис конструктора работает под `jupyter` из группы
`etldev`, а прод-репозиторий — в `etlprod`. Чтобы включить, нужно три вещи, и
все три на сервере `airflow`.

**1. Пустить пользователя сервиса в группу прода** (под root):

```bash
sudo usermod -aG etlprod jupyter
```

Он же прописан в `MEMBERS` в `setup-airflow-prod.sh`, так что повторный запуск
скрипта делает это сам — и заодно проверяет, видит ли `jupyter` интерпретатор
`/opt/airflow/venv/bin/python` с airflow. Это не мелочь: гейт `pre-receive`
выполняется **правами того, кто пушит**, и без доступа к venv push будет
отклонён.

**2. Перезапустить конструктор** (обязательно, а не «на всякий случай»):

```bash
sudo systemctl restart etl-dagbuilder-api
```

Список групп процесс читает **при старте**. Без рестарта `jupyter` уже в
`etlprod`, а работающий сервис — ещё нет.

**3. Добавить remote в клон конструктора** (от имени `jupyter` — клон его):

```bash
sudo -u jupyter git -C /opt/jupyter/workdir/konkin/etl \
     remote add prod /opt/airflow-prod/etl.git
```

**Локальный путь, а не `ssh://devel@airflow/...`.** Ssh-адрес в этом файле
написан для выкладки с dev-ПК — оттуда иначе никак. Здесь оба репозитория на
одной машине, и ssh означал бы ключ, ходящий под `devel`, то есть полные права
`devel` ради одной операции. По локальному пути push работает правами файловой
системы: bare-репо групповой (`core.sharedRepository group`, `etlprod`, 2775
setgid), `safe.directory` для него уже прописан системно — хватает членства в
группе.

Проверка: в конструкторе на вкладке «Версии и прод» нажать «Проверить доступ».
Она делает `git ls-remote` по той же дороге, что и push, и либо включает
кнопку, либо показывает точную причину.

**Чем за это платим.** Группа `etlprod` даёт `jupyter` и чтение
`/opt/airflow-prod/prod-src/.env` (режим 0640, группа `etlprod`) — то есть
реквизиты **боевых** БД. Если это неприемлемо, дайте доступ точечно, ACL только
на bare-репозиторий, и не добавляйте `jupyter` в группу:

```bash
sudo setfacl -R -m u:jupyter:rwX -m d:u:jupyter:rwX /opt/airflow-prod/etl.git
```

### Коллегам (Jupyter): ничего не меняется

Работа идёт только с тестом — прод коллеги не трогают вообще (bare-репо прода в
группе `etlprod`, туда пишет только `devel`, а с недавних пор ещё и конструктор,
если включена выкладка из него — см. выше).
