...
### Почему конструктор перезапускает сам себя

Он **живёт в том же репозитории, который выкладывает**:
`/opt/jupyter/workdir/konkin/etl` — это Jupyter-клон, и хук обновляет его на
каждом push в `test`. То есть push меняет не только код airflow, но и код
самого конструктора.

Дальше расходятся две половины. Страницу браузер берёт с диска — она сразу
новая. А python остался тем, что запустился: маршрутов из нового кода в нём нет,
и первый же новый вызов отвечает `404 нет маршрута`. Снаружи это выглядит как
сломанный интерфейс, хотя интерфейс-то как раз свежий.

Поэтому перезапуск конструктора — не побочный эффект, а часть выкладки. И
именно поэтому синхронный `sudo systemctl restart` из хука не годится: хук
работает внутри процесса конструктора, и тот убил бы себя вместе с ответом на
запрос.

### Как убедиться, что перезапуск состоялся

**В конструкторе** — в шапке, рядом с состоянием клона:

```
перезапуск  09:33:14 (2 мин назад)        ← навести: время по каждому сервису
перезапуск  запрошен в 09:33:11 — ждём…   ← опрос раз в 3 с, пока не сбудется
```

Пока перезапуск ожидается, страница спрашивает сервис раз в три секунды; когда
ждать нечего — раз в минуту. Постоянный частый опрос — это тысячи запросов за
день ради события, которое случается несколько раз.

Своё время старта конструктор берёт у себя, а не у systemd: процесс знает о себе
точно, и это работает даже там, где юнита нет вовсе.

**С сервера** — то же самое и подробнее:
