...
def fetch_prod_tags(root=ROOT, timeout=20):
    """Забрать теги prod-* из прод-репозитория. -> (сколько теперь, что сказал git).

    Зачем это отдельным действием. Тег выкладки ставит `deploy/deploy-prod.sh`
    в ТОМ клоне, из которого его запустили, и пушит в прод-репозиторий. Клон
    конструктора при этом ни при чём: выкладку делали с dev-ПК или по ssh — и у
    него тегов нет вовсе, а значит список «Выкладки прода» пуст, и откатываться
    некуда. Выглядит это как «функции нет», хотя функция есть, а данных нет.

    Сеть трогаем ТОЛЬКО по этому вызову: список версий открывают часто, и лазить
    за тегами на каждое открытие незачем.
    """
    before = len(prod_tags(limit=10000, root=root))
    rc, out, err = _git(root, "fetch", "--tags", "--quiet", "prod",
                        timeout=timeout)
    if rc != 0:
        # Нет remote 'prod' или до него не дойти — не беда: покажем, что есть
        # локально, и скажем почему. Отказывать в списке из-за этого нельзя.
        return len(prod_tags(limit=10000, root=root)), (err or out).strip()
    return len(prod_tags(limit=10000, root=root)), ""
