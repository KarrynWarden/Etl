...
def _complex_ui():
    ...
    group_help = W.HTML(
        ...
        "Даг со своей логикой (не только перечисление линий) конструктор не "
        "переписывает — такой помечен «написан руками», и для него показывается "
        "строка, которую нужно вписать самому. Сейчас таких нет: "
        "<code>MocheckOrclPost</code> и <code>MedreeOrclPost</code> переведены "
        "в общий формат.</span>")
