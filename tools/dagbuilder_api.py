...
def _selftest():
    ...
    # ── «выкладывать нечего» ≠ «выложено» ────────────────────────────────
    # deploy-prod.sh выходит с НУЛЁМ и когда прод уже в нужном состоянии: это
    # не ошибка. Но интерфейс показывал такой исход как «Выложено» — человек
    # несколько раз подряд видел успех, а на проде не появилось ни коммита, ни
    # тега, и понять это было нечем.
    assert G._deploy_outcome(
        "== синхронизация ссылок ==\n"
        "Прод уже содержит это состояние — выкладывать нечего."
    ) == {"deployed": False, "tag": ""}
    assert G._deploy_outcome("Нечего выкладывать — не задано ни линий, ни путей.") \
        == {"deployed": False, "tag": ""}
    done = G._deploy_outcome(
        "== что уедет на прод ==\n"
        " * [new tag] prod-20260826-1030 -> prod-20260826-1030")
    assert done == {"deployed": True, "tag": "prod-20260826-1030"}, done
    # у двух выкладок в одну минуту тег с суффиксом — берём последний
    many = G._deploy_outcome("prod-20260826-1030\nprod-20260826-1030-2")
    assert many["tag"] == "prod-20260826-1030-2", many
