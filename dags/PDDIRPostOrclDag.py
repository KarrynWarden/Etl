...
def do_etl():
    try:
        logging.info('Выполнение Dag - PddirPostOrcl') # заголовок процесса !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
        tableNameMaster = 'pddir'
        dbMaster = 'Post'
        dbSlave = 'Orcl'
...
with DAG(dag_id='PddirPostOrcl', # id дага !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
         default_args=args,
         max_active_runs=1,
         tags=["PostOrcl", "pddir", "DbSync", "A55"], # тэги дага !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
