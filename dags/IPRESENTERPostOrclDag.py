...
def do_etl():
    try:
        logging.info('Выполнение Dag - IpresenterPostOrcl') # заголовок процесса !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
        tableNameMaster = 'ipresenter' # имя ведущей таблицы процесса !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
        dbMaster = 'Post'
        dbSlave = 'Orcl'
...
with DAG(dag_id='IpresenterPostOrcl', # id дага !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
         default_args=args,
         max_active_runs=1,
         tags=["PostOrcl", "ipresenter", "DbSync", "A56"], # тэги дага !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
