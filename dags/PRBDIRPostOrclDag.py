...
def do_etl():
    try:
        logging.info('Выполнение Dag - PrbdirPostOrcl') # заголовок процесса !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
        tableNameMaster = 'prbdir'
        dbMaster = 'Post'
        dbSlave = 'Orcl'
...
with DAG(dag_id='PrbdirPostOrcl', # id дага !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
         default_args=args,
         max_active_runs=1,
         tags=["PostOrcl", "prbdir", "", "DbSync", "A55"], # тэги дага !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
