...
def do_etl():
    try:
        logging.info('Выполнение Dag - IaddressPostOrcl') # заголовок процесса !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
        tableNameMaster = 'iaddress' # имя ведущей таблицы процесса !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
        dbMaster = 'Post'
        dbSlave = 'Orcl'
...
with DAG(dag_id='IaddressPostOrcl', # id дага !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
         default_args=args,
         max_active_runs=1,
         tags=["PostOrcl", "iaddress", "DbSync", "A56"], # тэги дага !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
