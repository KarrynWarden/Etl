...
def do_etl():
    try:
        logging.info('Выполнение Dag - IdeadPostOrcl') # заголовок процесса !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
        tableNameMaster = 'idead' # имя ведущей таблицы процесса !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
        dbMaster = 'Post'
        dbSlave = 'Orcl'
...
with DAG(dag_id='IdeadPostOrcl', # id дага !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
         default_args=args,
         max_active_runs=1,
         tags=["PostOrcl", "idead", "DbSync", "A56"], # тэги дага !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
