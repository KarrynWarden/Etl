...
"""DAG для ручного размораживания всех активных DAG'ов после исправления
системной ошибки.
...
def _getAllFatalDagRuns(**context):
    ...
    Критерии:
    1. DAG активен и имеет тег 'DbSync'
    2. Запуск в состоянии 'running' или 'failed'
    3. ВСЕ задачи в пуле Etl имеют error_class='fatal'
    4. Нет задач с error_class='record' или 'retryable'
    """
    with create_session() as session:
        logger = logging.getLogger("airflow.task")

        # 1. Получаем список DAG'ов с тегом 'DbSync'
        active_dag_ids = (
            session.query(DagModel.dag_id)
            .filter(
                DagModel.is_active == True,
                DagModel.dag_id != 'unfreeze_all_dags'
            )
            .join(DagModel.tags)
            .filter(DagTag.name == 'DbSync')
            .all()
        )
        active_dag_ids = [row[0] for row in active_dag_ids]

        if not active_dag_ids:
            logger.info("❌ Не найдено активных DAG'ов с тегом 'DbSync'")
            return []

        logger.info(f"✅ Активные DAG'и с тегом DbSync: {len(active_dag_ids)}")
...
def _printResults(**context):
    ...
    logger.info("=" * 60)
    logger.info("📊 РЕЗУЛЬТАТЫ РАЗМОРОЗКИ")
    logger.info("=" * 60)
...
# ----------------------------------------------------------------------------
with DAG(
    dag_id="unfreeze_all_dags",
    default_args=DEFAULT_ARGS,
    max_active_runs=1,
    tags=["system", "maintenance", "watcher", "DbSync"],
