...
def do_etl():
    try:
        logging.info('Выполнение Dag - ReqprepsmoPostOrcl') # заголовок процесса !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
        tableNameMaster = 'reqprepsmo'
        dbMaster = 'Post'
        dbSlave = 'Orcl'
...
with DAG(dag_id='ReqprepsmoPostOrcl', # id дага !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
         default_args=args,
         max_active_runs=1,
         tags=["PostOrcl", "reqprepsmo", "", "DbSync", "A55"], # тэги дага !ИЗМЕНИТЬ ДЛЯ НОВЫХ ДАГОВ!
