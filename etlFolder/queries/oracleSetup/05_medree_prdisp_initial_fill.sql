...
--------------------------------------------------------------------------------
-- Права переносу. Ходит он под etl_user, а таблица принадлежит koknaev —
-- без этой строки линия падает на ORA-00942 «table or view does not exist»,
-- и по тексту ошибки не догадаться, что дело в правах, а не в имени.
--
-- SELECT достаточно: medree_prdisp здесь ВЕДУЩАЯ, перенос её только читает.
--------------------------------------------------------------------------------
GRANT SELECT ON koknaev.medree_prdisp TO etl_user;
