...
- **Аудит занимает весь пул `Test`** (`AUDIT_POOL_SLOTS`, = размеру пула).
  Пока идёт аудит любой линии, ETL-переносы (`pool_slots=1`) ждут — аудит
  не пометит недоношенный перенос как успешный. `freeze_watcher` сидит в
  `default_pool`, поэтому замороженные ETL-даги слоты `Test` не держат и
  старт аудита не блокируют.
